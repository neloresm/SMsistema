...
/* ------------------------- segurança de senha -------------------------- */
function novoSalt() {
  const a = new Uint8Array(16); (crypto.getRandomValues ? crypto : window.crypto).getRandomValues(a);
...
/* ------------------------------ autenticação --------------------------- */
function Auth({ firstUser, onLogin, onRegister }) {
  const [modo, setModo] = useState(firstUser ? "cadastro" : "login");
  const [id, setId] = useState("");
  const [senha, setSenha] = useState("");
  const [reg, setReg] = useState({ nome: "", ident: "", senha: "", confirmar: "" });
  const [erro, setErro] = useState("");
  const [ok, setOk] = useState("");
  const [busy, setBusy] = useState(false);

  const entrar = async () => {
    if (!id || !senha) { setErro("Informe usuário e senha."); return; }
    setBusy(true); setErro(""); setOk("");
    const e = await onLogin(id.trim(), senha);
    if (e) { setErro(e); setBusy(false); }
  };
  const criar = async () => {
    if (!reg.nome || !reg.ident || !reg.senha) { setErro("Preencha nome, e-mail/usuário e senha."); return; }
    if (reg.senha.length < 4) { setErro("A senha deve ter ao menos 4 caracteres."); return; }
    if (reg.senha !== reg.confirmar) { setErro("As senhas não coincidem."); return; }
    setBusy(true); setErro(""); setOk("");
    const r = await onRegister({ nome: reg.nome, ident: reg.ident, senha: reg.senha });
    setBusy(false);
    if (r && r.error) { setErro(r.error); return; }
    // se aprovado (1º usuário/admin) o App já faz login e desmonta esta tela
    if (r && r.aprovado === false) {
      setOk("Cadastro criado! Aguarde a aprovação do administrador para acessar.");
      setModo("login"); setReg({ nome: "", ident: "", senha: "", confirmar: "" });
    }
  };
      ...
      <div className="auth-card">
        ...
        {modo === "login" ? (
          <>
            ...
            <label className="auth-field"><span>Usuário ou e-mail</span><input value={id} onChange={(e) => setId(e.target.value)} autoCapitalize="none" onKeyDown={onKey(entrar)} /></label>
          ...
          <>
            <div className="auth-h">{firstUser ? "Primeiro acesso — crie sua conta (administrador)" : "Criar conta"}</div>
            <label className="auth-field"><span>Nome</span><input value={reg.nome} onChange={(e) => setReg({ ...reg, nome: e.target.value })} /></label>
            <label className="auth-field"><span>E-mail ou usuário</span><input value={reg.ident} onChange={(e) => setReg({ ...reg, ident: e.target.value })} autoCapitalize="none" /></label>
            <label className="auth-field"><span>Senha</span><input type="password" value={reg.senha} onChange={(e) => setReg({ ...reg, senha: e.target.value })} /></label>
            ...
            {!firstUser && <button className="auth-link" onClick={() => { setModo("login"); setErro(""); setOk(""); }}>Já tenho conta — Entrar</button>}
            {!firstUser && <div className="auth-note">Novos cadastros podem precisar de aprovação do administrador.</div>}
          </>
        )}
      </div>
    </div>
...
/* ------------------------- gestão de usuários -------------------------- */
function UsersView({ users, sessionId, onAdd, onRole, onApprove, onReset, onDelete }) {
  const [f, setF] = useState({ nome: "", login: "", email: "", senha: "", role: "Usuário" });
  const [erro, setErro] = useState(""); const [busy, setBusy] = useState(false);
  const [resetId, setResetId] = useState(""); const [novaSenha, setNovaSenha] = useState("");
  const add = async () => { setBusy(true); setErro(""); const e = await onAdd(f); setBusy(false); if (e) { setErro(e); return; } setF({ nome: "", login: "", email: "", senha: "", role: "Usuário" }); };
  const salvarReset = async (id) => { if (!novaSenha) return; await onReset(id, novaSenha); setResetId(""); setNovaSenha(""); };
  const pendentes = users.filter((u) => u.aprovado === false).length;
  return (
    <section className="wrap">
      {pendentes > 0 && <div className="aviso">{pendentes} cadastro(s) aguardando aprovação.</div>}
      <div className="card">
        <div className="card-h">Novo usuário</div>
        <div className="grid">
          <label className="field"><span>Nome</span><input value={f.nome} onChange={(e) => setF({ ...f, nome: e.target.value })} /></label>
          <label className="field"><span>Usuário (login)</span><input value={f.login} onChange={(e) => setF({ ...f, login: e.target.value })} autoCapitalize="none" /></label>
          <label className="field"><span>E-mail (opcional)</span><input type="email" value={f.email} onChange={(e) => setF({ ...f, email: e.target.value })} autoCapitalize="none" /></label>
          <label className="field"><span>Senha</span><input type="password" value={f.senha} onChange={(e) => setF({ ...f, senha: e.target.value })} /></label>
          <label className="field"><span>Permissão</span><select value={f.role} onChange={(e) => setF({ ...f, role: e.target.value })}><option>Usuário</option><option>Administrador</option></select></label>
        </div>
        {erro && <div className="auth-erro">{erro}</div>}
        <div className="rep-tools"><button className="btn btn-gold" disabled={busy} onClick={add}>{busy ? "Salvando…" : "Adicionar usuário"}</button></div>
      </div>
      <div className="card">
        <div className="card-h">Usuários ({users.length})</div>
        <div className="tbl-wrap"><table className="tbl">
          <thead><tr><th>Nome</th><th>Usuário</th><th>E-mail</th><th>Permissão</th><th>Situação</th><th>Ações</th></tr></thead>
          <tbody>{users.map((u) => (
            <tr key={u.id}>
              <td>{u.nome}{u.id === sessionId && <span className="muted small"> (você)</span>}</td>
              <td>{u.login}</td><td>{u.email || "—"}</td>
              <td><select className="mini-sel" value={u.role} onChange={(e) => onRole(u.id, e.target.value)}><option>Usuário</option><option>Administrador</option></select></td>
              <td>{u.aprovado === false ? <Badge tone="amber">pendente</Badge> : <Badge tone="green">aprovado</Badge>}</td>
              <td>
                {resetId === u.id ? (
                  <span className="row-inline-mini">
                    <input className="mini-inp" type="password" placeholder="nova senha" value={novaSenha} onChange={(e) => setNovaSenha(e.target.value)} />
                    <button className="btn btn-mini" onClick={() => salvarReset(u.id)}>salvar</button>
                    <button className="btn btn-mini" onClick={() => { setResetId(""); setNovaSenha(""); }}>cancelar</button>
                  </span>
                ) : (
                  <span className="row-inline-mini">
                    {u.aprovado === false && <button className="btn btn-mini aprovar" onClick={() => onApprove(u.id)}>aprovar</button>}
                    <button className="btn btn-mini" onClick={() => { setResetId(u.id); setNovaSenha(""); }}>redefinir senha</button>
                    {u.id !== sessionId && <button className="btn btn-mini" onClick={() => onDelete(u.id)}>excluir</button>}
                  </span>
                )}
              </td>
            </tr>
...
export default function App() {
  ...
  const [session, setSession] = useState(null);   // usuário logado
  const [authReady, setAuthReady] = useState(false);

  const SHARED_KEY = "sm_sistema_db_v1";           // base compartilhada (mesmo banco para todos os usuários)
  const SESSION_KEY = "sm_sistema_session_v1";     // sessão local do aparelho
  ...
  useEffect(() => {
    (async () => {
      let loaded = null;
      try { const r = await window.storage.get(SHARED_KEY, true); if (r && r.value) loaded = JSON.parse(r.value); } catch (e) {}
      if (!loaded) { try { const r2 = await window.storage.get("orion_db_v3"); if (r2 && r2.value) loaded = JSON.parse(r2.value); } catch (e) {} }
      const base = normalizeDb(loaded || SEED);
      setDb(base); dbRef.current = JSON.stringify(base);
      // sessão salva neste aparelho
      try { const s = await window.storage.get(SESSION_KEY); if (s && s.value) { const uid0 = JSON.parse(s.value); const u = (base.users || []).find((x) => x && x.id === uid0); if (u) setSession(u); } } catch (e) {}
      setAuthReady(true);
    })();
  }, []);

  /* salvar (write-through) na base compartilhada a cada mudança */
  ...
  useEffect(() => {
    ...
    (async () => { try { await window.storage.set(SHARED_KEY, json, true); } catch (e) {} })();
  }, [db, authReady]);

  /* sondagem: puxa atualizações de outros usuários (aprox. tempo real) quando nada está aberto */
  useEffect(() => {
    if (!authReady || !session) return;
    const t = setInterval(async () => {
      if (form || aberto || novo || view === "usuarios") return;         // não interromper edição
      if (Date.now() - lastWrite.current < 3000) return;                  // evita eco da própria gravação
      try {
        const r = await window.storage.get(SHARED_KEY, true);
        if (r && r.value && r.value !== dbRef.current) {
          dbRef.current = r.value;
          setDb(normalizeDb(JSON.parse(r.value)));
        }
      } catch (e) {}
    }, 5000);
    return () => clearInterval(t);
  }, [authReady, session, form, aberto, novo, view]);

  const isAdmin = session && session.role === "Administrador";

  /* ---- autenticação ---- */
  const firstUser = authReady && (!(db.users || []).length);
  const doLogin = async (identificador, senha) => {
    const idn = lc(identificador);
    const u = (db.users || []).find((x) => x && (lc(x.login) === idn || (x.email && lc(x.email) === idn)));
    if (!u) return "Usuário não encontrado.";
    if (u.aprovado === false) return "Cadastro aguardando aprovação do administrador.";
    let h;
    try { h = await hashSenha(senha, u.salt); } catch (e) { return "Não foi possível verificar a senha neste navegador."; }
    if (h !== u.senhaHash) return "Senha incorreta.";
    setSession(u);
    try { await window.storage.set(SESSION_KEY, JSON.stringify(u.id)); } catch (e) {}
    return null;
  };
  const doRegister = async ({ nome, ident, senha }) => {
    nome = (nome ?? "").toString().trim(); senha = (senha ?? "").toString();
    const login = (ident ?? "").toString().trim();
    if (!nome || !login || !senha) return { error: "Preencha todos os campos." };
    if ((db.users || []).some((u) => u && (lc(u.login) === lc(login) || (u.email && lc(u.email) === lc(login))))) return { error: "Já existe um usuário com esse e-mail/usuário." };
    const first = !((db.users || []).length);
    let salt, senhaHash;
    try { salt = novoSalt(); senhaHash = await hashSenha(senha, salt); } catch (e) { return { error: "Não foi possível criar a conta neste navegador." }; }
    const u = { id: uid(), nome, login, email: login.includes("@") ? login : "", role: first ? "Administrador" : "Usuário", aprovado: first, salt, senhaHash, criadoEm: today() };
    setDb((p) => ({ ...p, users: [...(p.users || []), u] }));
    if (first) { setSession(u); try { await window.storage.set(SESSION_KEY, JSON.stringify(u.id)); } catch (e) {} }
    return { aprovado: u.aprovado };
  };
  const logout = async () => { setSession(null); try { await window.storage.delete(SESSION_KEY); } catch (e) {} };

  /* ---- gestão de usuários (somente admin) ---- */
  const addUser = async ({ nome, login, email, senha, role }) => {
    nome = (nome ?? "").toString(); login = (login ?? "").toString().trim(); email = (email ?? "").toString().trim(); senha = (senha ?? "").toString();
    if (!nome || !login || !senha) return "Preencha nome, usuário e senha.";
    if ((db.users || []).some((u) => u && lc(u.login) === lc(login))) return "Já existe um usuário com esse login.";
    let salt, senhaHash;
    try { salt = novoSalt(); senhaHash = await hashSenha(senha, salt); } catch (e) { return "Não foi possível criar a conta neste navegador."; }
    setDb((p) => ({ ...p, users: [...(p.users || []), { id: uid(), nome: nome.trim(), login, email, role: role || "Usuário", aprovado: true, salt, senhaHash, criadoEm: today() }] }));
    return null;
  };
  const setUserRole = (id, role) => setDb((p) => ({ ...p, users: p.users.map((u) => (u.id === id ? { ...u, role } : u)) }));
  const aprovarUser = (id) => setDb((p) => ({ ...p, users: p.users.map((u) => (u.id === id ? { ...u, aprovado: true } : u)) }));
  const resetUserSenha = async (id, senha) => { const salt = novoSalt(); const senhaHash = await hashSenha(senha, salt); setDb((p) => ({ ...p, users: p.users.map((u) => (u.id === id ? { ...u, salt, senhaHash } : u)) })); };
  const delUser = (id) => setDb((p) => ({ ...p, users: p.users.filter((u) => u.id !== id) }));
  ...
  const salvar = (d) => {
    ...
    setDb((p) => {
      ...
      linkKeysOf(d.tipo).forEach((k) => { const nm = (d[k] ?? "").toString().trim(); if (nm && !nameSet.has(lc(nm)) && lc(nm) !== lc(d.nome)) { list.push(stubAnimal(nm, session && session.nome)); nameSet.add(lc(nm)); } });
      const exists = list.some((a) => a.id === d.id);
      if (exists) list = list.map((a) => (a.id === d.id ? d : a));
      else list = [{ ...d, historico: [{ id: uid(), data: today(), tipo: "Cadastro criado", desc: "Ficha criada", responsavel: session && session.nome }, ...(d.historico || [])] }, ...list];
  ...
  const quickAnimal = (nm) => { nm = (nm ?? "").toString().trim(); if (!nm) return; setDb((p) => (p.ativos.some((a) => a && a.tipo === "animal" && lc(a.nome) === lc(nm)) ? p : { ...p, ativos: [...p.ativos, stubAnimal(nm, session && session.nome)] })); };
  ...
  if (!session) return <Auth firstUser={firstUser} onLogin={doLogin} onRegister={doRegister} />;
    ...
    <div className="app">
      ...
      <aside className={`side ${navOpen ? "open" : ""}`}>
        ...
        <div className="side-foot">
          <div className="user-box"><div className="user-name">{session.nome}</div><div className="user-role">{session.role}</div></div>
      ...
      <main className="main">
        ...
        {view === "usuarios" && isAdmin && (
          <UsersView users={db.users || []} sessionId={session.id} onAdd={addUser} onRole={setUserRole} onApprove={aprovarUser} onReset={resetUserSenha} onDelete={delUser} />
        )}
      </main>
